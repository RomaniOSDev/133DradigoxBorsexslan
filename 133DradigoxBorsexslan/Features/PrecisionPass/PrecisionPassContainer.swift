//
//  PrecisionPassContainer.swift
//  133DradigoxBorsexslan
//

import SwiftUI

struct PrecisionPassContainer: View {
    @EnvironmentObject private var progress: GameProgressStore
    @StateObject private var model: PrecisionPassViewModel
    ...
    init(difficulty: Difficulty, path: Binding<NavigationPath>) {
        ...
        _path = path
        _model = StateObject(wrappedValue: PrecisionPassViewModel(difficulty: difficulty))
    }

    var body: some View {
        ZStack {
            PrecisionPassView(model: model) {
                guard !showResult else { return }
                let slot = ChallengeSlot(activity: .precisionPass, difficulty: difficulty)
