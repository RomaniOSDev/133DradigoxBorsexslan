//
//  VirtualMarathonContainer.swift
//  133DradigoxBorsexslan
//

import SwiftUI

struct VirtualMarathonContainer: View {
    @EnvironmentObject private var progress: GameProgressStore
    @StateObject private var model: VirtualMarathonViewModel
    ...
    init(difficulty: Difficulty, path: Binding<NavigationPath>) {
        ...
        _path = path
        _model = StateObject(wrappedValue: VirtualMarathonViewModel(difficulty: difficulty))
    }

    var body: some View {
        ZStack {
            VirtualMarathonView(model: model) {
                guard !showResult else { return }
                let slot = ChallengeSlot(activity: .virtualMarathon, difficulty: difficulty)
